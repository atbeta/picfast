<template>
	<div class="max-w-lg">
		<h2 class="text-lg font-semibold text-[var(--color-text-primary)] mb-4">个人设置</h2>
		<n-spin :show="loading">
			<n-form label-placement="left" label-width="100">
				...
				<n-form-item label="默认策略">
					<n-select v-model:value="form.defaultStrategy" :options="strategyOptions" placeholder="跟随组默认" clearable />
				</n-form-item>
...
import { NForm, NFormItem, NInput, NSelect, NButton, NDivider, NSpin, useMessage } from 'naive-ui'
...
const form = reactive({ name: '', password: '', defaultStrategy: 0 as number })
...
onMounted(async () => {
	...
	try {
		...
		if (settings.default_strategy) form.defaultStrategy = Number(settings.default_strategy)
...
async function save() {
	...
	try {
		...
		data.settings = form.defaultStrategy && form.defaultStrategy !== 0 ? { default_strategy: form.defaultStrategy } : { default_strategy: null }
