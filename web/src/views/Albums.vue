<template>
  <div>
    ...
    <n-spin :show="loading">
      ...
      <n-empty v-if="!loading && albums.length === 0" description="暂无相册，点击上方按钮创建" style="margin-top: 48px;" />
    </n-spin>

    <!-- 相册内图片 -->
    <n-modal v-model:show="albumImagesVisible" preset="card" :title="currentAlbum?.name + ' - 图片列表'" style="width: 700px;">
      <n-spin :show="albumImagesLoading">
        <n-grid :cols="4" :x-gap="8" :y-gap="8">
          <n-gi v-for="img in albumImages" :key="img.key">
            <n-image :src="img.thumbnail_url || img.url" width="100%" height="100" object-fit="cover" preview-disabled />
          </n-gi>
        </n-grid>
    ...
    <!-- 创建 -->
    <n-modal v-model:show="showCreate" preset="dialog" title="新建相册" positive-text="创建" negative-text="取消"
    ...
    <!-- 编辑 -->
    <n-modal v-model:show="showEdit" preset="dialog" title="编辑相册" positive-text="保存" negative-text="取消"
...
import {
  ...
  NModal, NForm, NFormItem, NInput, NImage, useMessage, useDialog,
} from 'naive-ui'
