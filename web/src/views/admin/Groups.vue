<template>
  <div>
    ...
    <n-modal v-model:show="showModal" preset="dialog" :title="editing ? '编辑分组' : '新建分组'"
      positive-text="保存" negative-text="取消" @positive-click="saveGroup" style="width: 500px;">
...
const form = reactive({ name: '', is_default: false, max_size: 5, extensions: 'jpg,jpeg,png,gif,webp,bmp,svg', limit_per_day: 300, limit_per_month: 9999 })

const columns: DataTableColumns = [
  { title: 'ID', key: 'id', width: 60 },
  { title: '名称', key: 'name' },
  { title: '默认', key: 'is_default', width: 60, render: (row: any) => h(NTag, { type: row.is_default ? 'success' : 'default', size: 'small' }, () => row.is_default ? '是' : '否') },
  ...
  { title: '每日上限', key: 'limit', width: 80, render: (row: any) => `${row.configs?.limit_per_day || '-'} 张` },
  {
    ...
    render: (row: any) =>
      h(NSpace, { size: 'small' }, () => [
        ...
        h(NButton, { size: 'small', type: 'error', onClick: () => deleteGroup(row), disabled: row.is_default }, () => '删除'),
      ]),
  },
]
