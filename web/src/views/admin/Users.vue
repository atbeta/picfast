...
const columns: DataTableColumns = [
  ...
  { title: '昵称', key: 'name' },
  { title: '邮箱', key: 'email' },
  { title: '角色', key: 'role', width: 80, render: (row: any) => h(NTag, { type: row.role === 'admin' ? 'info' : 'default', size: 'small' }, () => row.role === 'admin' ? '管理员' : '用户') },
  { title: '状态', key: 'status', width: 80, render: (row: any) => h(NTag, { type: row.status === 1 ? 'success' : 'error', size: 'small' }, () => row.status === 1 ? '正常' : '冻结') },
