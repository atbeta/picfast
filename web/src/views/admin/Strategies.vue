<template>
  <div>
    ...
    <n-modal v-model:show="showModal" preset="dialog" :title="editing ? '编辑策略' : '新建策略'"
      positive-text="保存" negative-text="取消" @positive-click="saveStrategy" style="width: 550px;">
      <n-form label-placement="left" label-width="80">
        ...
        <template v-if="form.type === 'local'">
          <n-form-item label="存储路径"><n-input v-model:value="form.localRoot" placeholder="/data/images" /></n-form-item>
        </template>
        <template v-if="form.type === 's3'">
          <n-form-item label="Endpoint"><n-input v-model:value="form.s3Endpoint" placeholder="https://s3.amazonaws.com" /></n-form-item>
          ...
          <n-form-item label="Secret Key"><n-input v-model:value="form.s3SecretKey" type="password" show-password-on="click" /></n-form-item>
        </template>
      </n-form>
    </n-modal>
  </div>
...
const form = reactive({
  ...
  s3Endpoint: '', s3Region: 'us-east-1', s3Bucket: '', s3AccessKey: '', s3SecretKey: '',
})
...
async function fetchStrategies() {
  ...
  try {
    ...
    strategies.value = Array.isArray(d) ? d : (d?.items || [])
  } catch {
...
async function saveStrategy() {
  ...
  if (form.type === 'local') {
    ...
    configs = { endpoint: form.s3Endpoint, region: form.s3Region, bucket: form.s3Bucket, access_key: form.s3AccessKey, secret_key: form.s3SecretKey }
  }
  try {
    if (editing.value) {
