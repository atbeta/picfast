...
const SelectContext = React.createContext<{items?: Record<string, React.ReactNode>}>({});

function Select({ items, ...props }: React.ComponentProps<typeof SelectPrimitive.Root> & { items?: Record<string, React.ReactNode> }) {
  return (
    ...
    <SelectPrimitive.Portal>
      ...
      >
        <SelectPrimitive.Popup
          ...
          className={cn("relative isolate z-50 max-h-(--available-height) w-(--anchor-width) min-w-36 max-w-[min(90vw,28rem)] origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10", className )}
          {...props}
        >
